...
/** Create a reviewer analyzer function */
export const createReviewerAnalyzer = (
  client: ModelClient,
  policy: PolicyFile,
  sessionId: string,
): AnalyzeFn => {
  const analyze: AnalyzeFn = async (
    ...
    const system = buildReviewerPrompt(policy, context);
    ...
    try {
      ...
      console.error(
        "oversight: reviewer could not parse model response as findings JSON.",
      );
      return { findings: [], notes: [] };
    }
  };
