...
- Runs an interview to gather: project name, owner, purpose, primary language(s), key constraints
- Generates `VISION.md` skeleton from interview output
...
- `internal/init/` — interview + scaffold generation
- `internal/context/` — CLAUDE.md generation
- `internal/adr/` — ADR management
- `internal/tdd/` — TDD management
- `internal/config/` — .telesis/config.yml read/write
- `templates/` — embedded document templates

**Dependencies:** minimal
- `cobra` — CLI framework
- `viper` — config management
- `go:embed` — template embedding (no runtime file deps)
...
1. `telesis init` produces the full document structure from an interview
...
5. The Telesis repo itself is initialized with `telesis init`
6. Claude Code sessions on the Telesis repo use the generated `CLAUDE.md`
7. bop reviews at least one PR on the Telesis repo
...
The foundation document (`telesis-foundation.md`) becomes `docs/VISION.md`.
This MVP definition becomes the seed for `docs/PRD.md` and `docs/MILESTONES.md`.

---

## Parallel Track: Bop ACP Compliance

Independent of the Telesis MVP, bop gets an ACP server interface so Telesis
can drive it as a first-class agent in future swarm orchestration.

- bop exposes an ACP-compliant server endpoint
- Telesis (future orchestration layer, TypeScript) drives bop via `acpx`
- bop remains a standalone tool — ACP is additive, not replacing existing interfaces
